    ...
    <v-card>
      <v-toolbar dark color="primary">
        ...
        <v-toolbar-items v-if="alikhUtils.getCookieValue('userInfo').user_type =='ADMINISTRATOR'">
          <v-btn  dark text @click="ApproveFile"> Restore </v-btn>
        </v-toolbar-items>
...
export default {
  ...
  data() {
    return {
      alikhUtils,
      selectedFile: {},
      reviewsToShow: { value: "Deleted Files", key: "deleted" },
      reviewsDropdown: [
  ...
  computed: {
    tableValues() {
      ...
      try {
        if (this.reviewsToShow.key == "deleted") {
  ...
  methods: {
    ...
    ApproveFile() {
      ...
      this.restoreFile(this.selectedFile).then((data)=>{
        if (data.httpSuccess) {
          alikhUtils.successToast(`Restored ${this.selectedFile.name}`)
    ...
    ...mapActions("files", ["getFiles","updateFile", "restoreFile"]),
  },
  created() {
    this.getFilesFromServer();
  },
